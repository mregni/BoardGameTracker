...
export const SettingsToggle = ({ field, label, description, disabled = false }: Props) => {
	...
	return (
		<div className={`flex items-center justify-between p-3 bg-background rounded-lg border border-white/10 gap-3 ${disabled ? "opacity-50 cursor-not-allowed" : ""}`}>
			<Switch.Root
			...
			>
				<Switch.Thumb className={`inline-block h-5 w-5 transform rounded-full bg-white transition-transform ${checked ? "translate-x-6" : "translate-x-1"}`} />
			</Switch.Root>
