...
export const WinnerCount = ({ row, position, playerLeft, playerRight, className, ...props }: RowProps) => {
  ...
  const translationKey = isWinner ? 'compare.wins.winner' : 'compare.wins.looser';
  ...
  return (
    <div className={cx('flex flex-row gap-2 items-start h-10', className)} {...props}>
      ...
      <div className="flex flex-col">
        <span>{t(translationKey, translationParams)}</span>
