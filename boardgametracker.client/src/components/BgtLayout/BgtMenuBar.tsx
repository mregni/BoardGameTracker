...
import {
  Bars3Icon, ChevronDoubleLeftIcon, ChevronDoubleRightIcon, XMarkIcon,
...
useEffect(() => {
  ...
  return (
    <div className='flex-col flex justify-between md:hidden'>
      ...
      <div className={clsx(
        "mobile-menu bg-gray-950 absolute w-full top-16 z-40",
        !open && "hidden-menu")}>
        {
          menuItems.map(x => <BgtMenuItem fullSize={true} key={x.path} item={x} count={counts.find(y => y.key == x.path)?.value} />)
        }
      </div>
    </div>
  )
    ...
    <>
      <div className={
        ...
        <div className="px-4 flex flex-col">
          ...
          <div className="mt-6">
            {
              menuItems.map(x => <BgtMenuItem fullSize={fullSizeMenu} key={x.path} item={x} count={counts.find(y => y.key == x.path)?.value} />)
            }
          </div>
        </div>
        <div className={
